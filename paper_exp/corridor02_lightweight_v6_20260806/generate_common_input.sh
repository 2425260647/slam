...
export ROS_LOG_DIR="${workspace}/.ros_runtime/corridor02_v6_projection"
...
for attempt in $(seq 1 80); do
  if rosnode list 2>/dev/null | grep -qx \
      '/m3dgr_mid360_cloud_to_scan'; then
    break
  fi
  sleep 0.25
done
rosnode list | grep -qx '/m3dgr_mid360_cloud_to_scan'
...
/usr/bin/time -f 'projection_play_wall_sec=%e' \
  ...
  rosbag play --clock --quiet --rate=1.0 "${source_bag}" \
...
odom_count=$(rosbag info --yaml "${output_bag}" | awk \
  '$1 == "-" && $2 == "topic:" && $3 == "/odom" {found=1; next} \
   found && $1 == "messages:" {print $2; exit}')
if [[ "${scan_count:-0}" -lt 2900 || "${odom_count:-0}" -lt 5800 ]]; then
  echo "Common input incomplete: scan=${scan_count:-0}, odom=${odom_count:-0}" >&2
  exit 3
fi
printf 'scan_count=%s\nodom_count=%s\n' "${scan_count}" "${odom_count}" \
  >"${exp_root}/common_input_counts.txt"
echo "Generated common input: scan=${scan_count}, odom=${odom_count}"
