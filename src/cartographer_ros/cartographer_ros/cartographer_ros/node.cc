...
Node::Node(
  ...
  slip_metric_publisher_ = node_handle_.advertise<::std_msgs::Float32>(
      "slip_metric", kLatestOnlyPublisherQueueSize);
  slip_state_publisher_ = node_handle_.advertise<::std_msgs::Bool>(
      "slip_state", kLatestOnlyPublisherQueueSize);
  odom_weight_scale_publisher_ = node_handle_.advertise<::std_msgs::Float32>(
      "odom_weight_scale", kLatestOnlyPublisherQueueSize);
  slip_lidar_reliability_publisher_ =
      node_handle_.advertise<::std_msgs::Float32>(
          "slip_lidar_reliability", kLatestOnlyPublisherQueueSize);
  // [Innovation 2] Stable per-Solve summaries prevent short anomaly events
  // [Innovation 2] from being hidden by the final normal edge in a replay.
  consistency_anomaly_trigger_count_publisher_ =
      node_handle_.advertise<::std_msgs::UInt32>(
          "consistency_anomaly_trigger_count", kLatestOnlyPublisherQueueSize);
  // [Innovation 2] This is the timestamp of the historical edge that caused
  // [Innovation 2] the latest state rising edge, not the later Solve() finish
  // [Innovation 2] time at which the ROS snapshot becomes visible.
  consistency_anomaly_time_publisher_ =
      node_handle_.advertise<::std_msgs::Float64>(
          "consistency_anomaly_time", kLatestOnlyPublisherQueueSize);
  odom_min_weight_scale_publisher_ =
      node_handle_.advertise<::std_msgs::Float32>(
          "odom_min_weight_scale", kLatestOnlyPublisherQueueSize);
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent& timer_event) {
  ...
  if (slip_metric.enabled) {
    ::std_msgs::Float32 slip_score_msg;
    slip_score_msg.data = static_cast<float>(slip_metric.slip_score);
    slip_metric_publisher_.publish(slip_score_msg);

    ::std_msgs::Bool slip_state_msg;
    slip_state_msg.data = slip_metric.slipping;
    slip_state_publisher_.publish(slip_state_msg);

    ::std_msgs::Float32 weight_scale_msg;
    weight_scale_msg.data = static_cast<float>(slip_metric.weight_scale);
    odom_weight_scale_publisher_.publish(weight_scale_msg);

    ::std_msgs::Float32 lidar_reliability_msg;
    lidar_reliability_msg.data =
        static_cast<float>(slip_metric.lidar_reliability);
    slip_lidar_reliability_publisher_.publish(lidar_reliability_msg);

    // [Innovation 2] These are summaries of the complete historical-edge
    // [Innovation 2] replay in the latest Solve(), not lifetime event counts.
    ::std_msgs::UInt32 trigger_count_msg;
    trigger_count_msg.data = slip_metric.anomaly_trigger_count;
    consistency_anomaly_trigger_count_publisher_.publish(trigger_count_msg);

    if (slip_metric.anomaly_trigger_count > 0) {
      // [Innovation 2] Publishing the edge timestamp allows event labels to be
      // [Innovation 2] matched independently of optimize_every_n_nodes.
      ::std_msgs::Float64 anomaly_time_msg;
      anomaly_time_msg.data = ToRos(slip_metric.latest_anomaly_time).toSec();
      consistency_anomaly_time_publisher_.publish(anomaly_time_msg);
    }

    ::std_msgs::Float32 minimum_weight_scale_msg;
    minimum_weight_scale_msg.data =
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (degeneracy_metric.enabled) {
      // [Innovation 1] The core metric is expressed in the local-SLAM frame
      // [Innovation 1] because that is the frame of the Ceres translation
      // [Innovation 1] residual. Rotate it into map_frame for ROS diagnostics
      // [Innovation 1] so offline plots do not apply the robot yaw twice.
      const Eigen::Vector3d direction_in_local(
          degeneracy_metric.direction.x(), degeneracy_metric.direction.y(), 0.);
      const Eigen::Vector3d direction_in_map =
          trajectory_data.local_to_map.rotation() * direction_in_local;
      ::geometry_msgs::Vector3 direction_msg;
      direction_msg.x = direction_in_map.x();
      direction_msg.y = direction_in_map.y();
      direction_msg.z = 0.;
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions& options) const {
  ...
  // use_odometry=false 时不会订阅 /odom；此时 Cartographer 仅靠激光和 TF/static
  // TF 估计轨迹。
...
void Node::HandleOdometryMessage(const int trajectory_id,
  ...
  // 同一份里程计既加入 PoseExtrapolator，也进入 Cartographer
  // trajectory_builder。
  absl::MutexLock lock(&mutex_);
