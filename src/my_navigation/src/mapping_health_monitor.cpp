...
public:
  MappingHealthMonitor(ros::NodeHandle &nh, ros::NodeHandle &pnh)
    ...
    pnh_.param<std::string>("local_grid_topic", local_grid_topic_,
                            "/local_occupancy_grid");
    pnh_.param<std::string>("map_frame", map_frame_, "map");
    pnh_.param<std::string>("base_frame", base_frame_, "base_link");
    pnh_.param<bool>("require_local_grid", require_local_grid_, true);
    ...
    scan_sub_ = nh_.subscribe(scan_topic_, 50,
                              &MappingHealthMonitor::scanCallback, this);
    map_sub_ =
        nh_.subscribe(map_topic_, 5, &MappingHealthMonitor::mapCallback, this);
    if (require_local_grid_) {
      local_grid_sub_ =
          nh_.subscribe(local_grid_topic_, 10,
                        &MappingHealthMonitor::localGridCallback, this);
    }
    report_timer_ = nh_.createTimer(ros::Duration(report_period_),
                                    &MappingHealthMonitor::report, this);

    ROS_INFO(
        "[MAPPING_HEALTH] scan=%s map=%s local_grid=%s tf=%s->%s period=%.1fs",
...
private:
  ...
  static double
  yawFromTransform(const geometry_msgs::TransformStamped &transform) {
    tf2::Quaternion q;
  ...
  void updatePoseSample(const ros::Time &now, double x, double y, double yaw) {
    ...
    while (!poses_.empty() &&
           (now - poses_.front().stamp).toSec() > pose_window_sec_) {
      poses_.pop_front();
    }
  }
  ...
  void computePoseWindow(double &span_xy, double &span_yaw) const {
    ...
    for (const PoseSample &pose : poses_) {
      ...
      span_yaw = std::max(
          span_yaw, std::abs(angleDistance(pose.yaw, poses_.front().yaw)));
    }
  ...
  void report(const ros::TimerEvent &) {
    ...
    const double map_age = last_map_wall_time_.isZero()
                               ? std::numeric_limits<double>::infinity()
                               : (now - last_map_wall_time_).toSec();
    const double local_age = last_local_grid_wall_time_.isZero()
                                 ? std::numeric_limits<double>::infinity()
    ...
    const double map_free_pct =
        map_total > 0 ? 100.0 * map_free_ / map_total : 0.0;
    const double map_occ_pct =
        map_total > 0 ? 100.0 * map_occupied_ / map_total : 0.0;
    const int local_total = local_unknown_ + local_free_ + local_occupied_;
    const double local_free_pct =
        local_total > 0 ? 100.0 * local_free_ / local_total : 0.0;
    const double local_occ_pct =
        local_total > 0 ? 100.0 * local_occupied_ / local_total : 0.0;

    if (ok) {
      ROS_INFO(
          "[MAPPING_HEALTH] OK scan=%.1fHz frame=%s finite=%d/%d map=%ux%u "
          "free=%.1f%% occ=%.1f%% local=%ux%u free=%.1f%% occ=%.1f%% "
          "pose=(%.3f, %.3f, yaw=%.3f) jitter%.0fs=%.3fm/%.3frad",
          scan_hz, last_scan_frame_.c_str(), last_scan_finite_,
          last_scan_points_, map_width_, map_height_, map_free_pct, map_occ_pct,
          local_grid_width_, local_grid_height_, local_free_pct, local_occ_pct,
          x, y, yaw, pose_window_sec_, pose_span_xy, pose_span_yaw);
    } else {
      ROS_WARN("[MAPPING_HEALTH] WARN scan=%s %.1fHz map=%s age=%.2fs local=%s "
               "age=%.2fs tf=%s pose=(%.3f, %.3f, yaw=%.3f) "
               "jitter%.0fs=%.3fm/%.3frad tf_error='%s'",
               scan_ok ? "OK" : "BAD", scan_hz, map_ok ? "OK" : "BAD", map_age,
               local_ok ? "OK" : "BAD", local_age, tf_ok ? "OK" : "BAD", x, y,
               yaw, pose_window_sec_, pose_span_xy, pose_span_yaw,
               tf_error.c_str());
    }
  }
