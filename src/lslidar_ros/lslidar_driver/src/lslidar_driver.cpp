...
/*        if (pointcloud_pub.getNumSubscribers() == 0) {
    ...
    void lslidarDriver::publishScan() {
        ...
        scan_msg->header.frame_id = "scan_link";
...
/*                TwoBytes raw_distance{0};
                ...
                firings.distance[blk_idx * 32 + scan_idx] =
                        static_cast<double>(raw_block.data[byte_idx] + raw_block.data[byte_idx + 1] * 256) * DISTANCE_RESOLUTION * distance_unit;
 ...
 */
    bool lslidarC16Driver::poll(void) {  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
        ...
        if (use_gps_ts) {
            ...
            //使用gps授时
            if ((timeStamp - timeStamp_bak).toSec() < 1e-9 && is_update_gps_time) {
                timeStamp = ros::Time(packet_time_s + 1, packet_time_ns);
            } else if ((timeStamp - timeStamp_bak).toSec() > 1.0 && is_update_gps_time) {
                timeStamp = ros::Time(packet_time_s - 1, packet_time_ns);
            }
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            if (1 == return_mode) {
                ...
                if (fabs(last_packet_timestamp) < 1e-6) {
                    ...
                    time = packet->stamp.toSec() -
                           (SCANS_PER_PACKET * 0.5 - (fir_idx - round((fir_idx / 32) * 0.5) * 32) - 1) * packet_interval_time;
                }
            }
            ...
            if (1 == return_mode) {
                ...
                sweep_end_time =
                        ...
                        static_cast<double>( SCANS_PER_PACKET * 0.5 -
                                             (end_fir_idx - round((end_fir_idx / 32) * 0.5) * 32)) * packet_interval_time;
            }
 ...
 */
    bool lslidarC32Driver::poll(void) {
        ...
        if (end_fir_idx != SCANS_PER_PACKET) {
            ...
            sweep_end_time = packet->stamp.toSec() - static_cast<double>(SCANS_PER_PACKET - end_fir_idx) *  packet_interval_time;
            sweep_end_time = sweep_end_time > 0 ? sweep_end_time : 0;
            publishPointcloud();
            if (publish_scan) publishScan();
